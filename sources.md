...
| `catalogue.results.seeDatabaseEntry` | ar de es fr as in `gallery.action.seeDatabaseEntry`, which it replaced | ar de es fr |
...
| `catalogue.search.overallDatabase` | ar de es fr as in `gallery.search.overallDatabase`, which it replaced | ar de es fr |
| `catalogue.search.placeholder` | ar de es fr as in `gallery.search.placeholder`, which it replaced | ar de es fr |
| `catalogue.search.relatedDatabase` | ar de es fr as in `gallery.search.relatedDatabase`, which it replaced | ar de es fr |
...
| `core.footer.aboutMwnf` | 3:footer1 (it pt tr); ar de es fr as in `gallery.footer.aboutMwnf`, which it replaced | ar de es fr it pt tr |
| `core.footer.contact` | 3:footer2 (fr it pt se tr); ar de es as in `gallery.footer.contact`, which it replaced | ar de es fr it pt se tr |
| `core.footer.cookies` | 3:cookie_policy (de es fr it pt se tr); ar as in `gallery.footer.cookies`, which it replaced | ar de es fr it pt se tr |
| `core.footer.copyright` | ar de es fr as in `gallery.footer.copyright`, which it replaced | ar de es fr |
| `core.footer.credits` | 3:footer4 (tr); ar de es fr as in `gallery.footer.credits`, which it replaced | ar de es fr tr |
| `core.footer.legalNotice` | de as in `gallery.footer.legalNotice`, which it replaced | de |
| `core.nav.about` | ar de es fr as in `gallery.nav.about`, which it replaced | ar de es fr |
| `core.nav.artisticIntroduction` | 3:home_artistic_intro (tr); ar de es fr as in `gallery.nav.artisticIntroduction`, which it replaced | ar de es fr tr |
| `core.nav.collection` | ar de es fr as in `gallery.nav.collection`, which it replaced | ar de es fr |
| `core.nav.credits` | 3:footer4 (tr); ar de es fr as in `gallery.nav.credits`, which it replaced | ar de es fr tr |
| `core.nav.home` | 3:back_to_section | ar es fr pt tr |
| `core.nav.myCollection` | ar de es fr as in `gallery.nav.myCollection`, which it replaced | ar de es fr |
| `core.nav.overallDatabase` | ar de es fr as in `gallery.nav.overallDatabase`, which it replaced | ar de es fr |
| `core.nav.partners` | 6:home_partnermuseums (tr); ar de es fr as in `gallery.nav.partners`, which it replaced | ar de es fr tr |
| `core.nav.timeline` | ar de es fr as in `gallery.nav.timeline`, which it replaced | ar de es fr |
...
| `core.section.about` | ar de es fr as in `gallery.section.about`, which it replaced | ar de es fr |
| `core.section.collection` | ar de es fr as in `gallery.section.collection`, which it replaced | ar de es fr |
| `core.section.credits` | 3:footer4 (tr); ar de es fr as in `gallery.section.credits`, which it replaced | ar de es fr tr |
...
| `layout.banner.detailFrom` | ar de es fr as in `gallery.banner.detailFrom`, which it replaced | ar de es fr |
| `partner.action.partnerProfile` | ar de es fr as in `gallery.action.partnerProfile`, which it replaced | ar de es fr |
...
| `partner.item.explorePartnerNote` | de as in `gallery.item.explorePartnerNote`, which it replaced | de |
| `partner.list.featured` | ar de es fr as in `gallery.partner.featured`, which it replaced | ar de es fr |
...
| `record.related.description` | ar de es fr as in `gallery.related.description`, which it replaced | ar de es fr |
| `record.related.exhibitions` | 13:sh_home_exhibitions | de it pt se tr |
| `record.related.linkPending` | ar de es fr as in `gallery.item.linkPending`, which it replaced | ar de es fr |
| `record.related.onDisplayIn` | 3:on_display_in | ar es fr |
| `record.related.timelineForItem` | 4:pc_view_hcr_item | ar es fr |
| `record.related.title` | ar de es fr as in `gallery.related.title`, which it replaced | ar de es fr |
...
The `gallery.*` rows trace the translations the shared entries carried
over. Those gallery entries were removed in 5.0.0, when nothing read them
any more; their text lives on in the entry that replaced them.
